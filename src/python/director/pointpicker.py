...
class PointPicker(object):
    ...
    def tick(self):
        ...
        if self.obj is None:
            ...
            tolerance = vis.getPickTolerance(self.view) * self.tolerance
            pickedPointFields = vis.pickPoint(self.lastMovePos, self.view, obj=self.obj, pickType=self.pickType, tolerance=tolerance)
            self.hoverPos = pickedPointFields.pickedPoint
...
class ObjectPicker(object):
    ...
    def tick(self):
        ...
        tolerance = vis.getPickTolerance(self.view) * self.tolerance
        pickedPointFields = vis.pickPoint(self.lastMovePos, self.view, pickType='cells', tolerance=tolerance,
                                              obj=objs)
        self.hoverPos = pickedPointFields.pickedPoint
        prop = pickedPointFields.pickedProp
